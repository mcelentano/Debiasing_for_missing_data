...
# Output files
filename.out = paste("../data/lambda_dependence",sim.time,sep="")
filename.tracking = paste("../sim_tracking/lambda_dependence",sim.time,sep="")

# Set parameters
N.replicates=10
delta = 1.5 # value used in Figure 1 of Jiang, Mukherjee, Sen, Sur
theta.d.0 = 0
theta.y.0 = 0
epsilon = .1 # overlap parameter
sigma = .2
lambda.y.seq = exp( seq( log(.3) , log(.01) , length.out = 25 ) )
lambda.d=.25
...
write.csv(
  experiment.df,
  paste("../data/lambda_dependence",sim.date,".csv",sep="")
)
